...
class Login extends Component {
    ...
    handleSubmit = event => {
        if(this.state.email === 'me@tuup.fi' && this.state.password ==='ok'){
            this.setState({
                toDashboard: true
            });
        }
        if(this.state.email !== 'me@tuup.fi'){
            this.setState({email_hint:'Please type: me@tuup.fi'});
        }
        else{
    ...
    render() {
        ...
        if (this.state.toDashboard === true) {
            return <Redirect to="/order_list" component={Order_List} />
        }
